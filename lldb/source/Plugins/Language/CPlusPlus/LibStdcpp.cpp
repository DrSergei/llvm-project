...
static bool
LibStdcppChronoTimePointSecondsSummaryProvider(ValueObject &valobj,
                                               Stream &stream,
                                               const TypeSummaryOptions &options,
                                               const char *fmt) {
  ValueObjectSP ptr_sp = valobj.GetChildMemberWithName("__d");
  if (!ptr_sp)
    return false;
  ptr_sp = ptr_sp->GetChildMemberWithName("__r");
  if (!ptr_sp)
    return false;
...
bool lldb_private::formatters::LibStdcppChronoSysSecondsSummaryProvider(
    ValueObject &valobj, Stream &stream, const TypeSummaryOptions &options) {
  return LibStdcppChronoTimePointSecondsSummaryProvider(valobj, stream, options,
                                                        "%FT%H:%M:%SZ");
}

bool lldb_private::formatters::LibStdcppChronoLocalSecondsSummaryProvider(
    ValueObject &valobj, Stream &stream, const TypeSummaryOptions &options) {
  return LibStdcppChronoTimePointSecondsSummaryProvider(valobj, stream, options,
                                                        "%FT%H:%M:%S");
}
...
static bool
LibStdcppChronoTimepointDaysSummaryProvider(ValueObject &valobj, Stream &stream,
                                           const TypeSummaryOptions &options,
                                           const char *fmt) {
  ValueObjectSP ptr_sp = valobj.GetChildMemberWithName("__d");
  if (!ptr_sp)
...
bool lldb_private::formatters::LibStdcppChronoSysDaysSummaryProvider(
    ValueObject &valobj, Stream &stream, const TypeSummaryOptions &options) {
  return LibStdcppChronoTimepointDaysSummaryProvider(valobj, stream, options,
                                                     "%FZ");
}

bool lldb_private::formatters::LibStdcppChronoLocalDaysSummaryProvider(
    ValueObject &valobj, Stream &stream, const TypeSummaryOptions &options) {
  return LibStdcppChronoTimepointDaysSummaryProvider(valobj, stream, options,
                                                     "%F");
}

bool lldb_private::formatters::LibStdcppChronoMonthSummaryProvider(
    ValueObject &valobj, Stream &stream, const TypeSummaryOptions &options) {
  ...
  ValueObjectSP ptr_sp = valobj.GetChildMemberWithName("_M_m");
  if (!ptr_sp)
    return false;
...
bool lldb_private::formatters::LibStdcppChronoWeekdaySummaryProvider(
    ValueObject &valobj, Stream &stream, const TypeSummaryOptions &options) {
  // FIXME: These are the names used in the C++20 ostream operator. Since LLVM
  // uses C++17 it's not possible to use the ostream operator directly.
  ...
  ValueObjectSP ptr_sp = valobj.GetChildMemberWithName("_M_wd");
  if (!ptr_sp)
    return false;
...
bool lldb_private::formatters::LibStdcppChronoYearMonthDaySummaryProvider(
    ValueObject &valobj, Stream &stream, const TypeSummaryOptions &options) {
  ValueObjectSP ptr_sp = valobj.GetChildMemberWithName("_M_y");
  if (!ptr_sp)
    return false;
  ptr_sp = ptr_sp->GetChildMemberWithName("_M_y");
  if (!ptr_sp)
    return false;
  int year = ptr_sp->GetValueAsSigned(0);

  ptr_sp = valobj.GetChildMemberWithName("_M_m");
  if (!ptr_sp)
    return false;
  ptr_sp = ptr_sp->GetChildMemberWithName("_M_m");
  if (!ptr_sp)
    return false;
  const unsigned month = ptr_sp->GetValueAsUnsigned(0);

  ptr_sp = valobj.GetChildMemberWithName("_M_d");
  if (!ptr_sp)
    return false;
  ptr_sp = ptr_sp->GetChildMemberWithName("_M_d");
  if (!ptr_sp)
    return false;
